...
const (
	Transactions_SendTransactions_FullMethodName   = "/seiprotocol.seistream.protobuf.Transactions/SendTransactions"
	Transactions_StreamTransactions_FullMethodName = "/seiprotocol.seistream.protobuf.Transactions/StreamTransactions"
)
...
type TransactionsClient interface {
	...
	StreamTransactions(ctx context.Context, in *SubscribeTransactionsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Transaction], error)
}
...
func (c *transactionsClient) StreamTransactions(ctx context.Context, in *SubscribeTransactionsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Transaction], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Transactions_ServiceDesc.Streams[1], Transactions_StreamTransactions_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeTransactionsRequest, Transaction]{ClientStream: stream}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Transactions_StreamTransactionsClient = grpc.ServerStreamingClient[Transaction]
...
type TransactionsServer interface {
	...
	// Get a stream of transactions
	StreamTransactions(*SubscribeTransactionsRequest, grpc.ServerStreamingServer[Transaction]) error
...
func (UnimplementedTransactionsServer) StreamTransactions(*SubscribeTransactionsRequest, grpc.ServerStreamingServer[Transaction]) error {
...
func _Transactions_StreamTransactions_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(SubscribeTransactionsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(TransactionsServer).StreamTransactions(m, &grpc.GenericServerStream[SubscribeTransactionsRequest, Transaction]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Transactions_StreamTransactionsServer = grpc.ServerStreamingServer[Transaction]
...
var Transactions_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "seiprotocol.seistream.protobuf.Transactions",
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _Transactions_StreamTransactions_Handler,
			ServerStreams: true,
		},
	},
